...
static void setupStateMachines() {
  ...
  Transition<SubState, Event> sub1_trans[] = {
      {SubState::S_IDLE, Event::Shoe0InitWet, SubState::S_WET,
       []() { FSM_DBG_PRINTLN("SUB1: entered S_WET"); }},
      {SubState::S_IDLE, Event::Shoe0InitDry, SubState::S_DRY,
       []() { FSM_DBG_PRINTLN("SUB1: entered S_DRY"); }},
      ...
      {SubState::S_DRY, Event::DryCheckFailed, SubState::S_WET,
       []() { FSM_DBG_PRINTLN("SUB1: Dry->Wet (DryCheckFailed)"); }},
  };

  Transition<SubState, Event> sub2_trans[] = {
      {SubState::S_IDLE, Event::Shoe1InitWet, SubState::S_WET,
       []() { FSM_DBG_PRINTLN("SUB2: entered S_WET"); }},
      {SubState::S_IDLE, Event::Shoe1InitDry, SubState::S_DRY,
       []() { FSM_DBG_PRINTLN("SUB2: entered S_DRY"); }},
      {SubState::S_WET, Event::SubStart, SubState::S_COOLING,
      ...
      {SubState::S_DRY, Event::DryCheckFailed, SubState::S_WET,
       []() { FSM_DBG_PRINTLN("SUB2: Dry->Wet (DryCheckFailed)"); }},
  ...
  fsmSub1.setRun(SubState::S_WET, []() {
    if (g_subWetStartMs[0] != 0 && (uint32_t)(millis() - g_subWetStartMs[0]) >= WET_TIMEOUT_MS) {
      FSM_DBG_PRINTLN("SUB1: WET timeout -> advance to COOLING");
      g_subWetStartMs[0] = 0;
      fsmSub1.handleEvent(Event::SubStart);
    }
  });
  fsmSub2.setRun(SubState::S_WET, []() {
    if (g_subWetStartMs[1] != 0 && (uint32_t)(millis() - g_subWetStartMs[1]) >= WET_TIMEOUT_MS) {
      FSM_DBG_PRINTLN("SUB2: WET timeout -> advance to COOLING");
      g_subWetStartMs[1] = 0;
      fsmSub2.handleEvent(Event::SubStart);
    }
  });
  ...
  fsmSub1.setRun(SubState::S_COOLING, []() {
    if (g_subCoolingStartMs[0] == 0)
      return;
    if ((uint32_t)(millis() - g_subCoolingStartMs[0]) < DRY_COOL_MS)
      return;
  ...
  fsmSub2.setRun(SubState::S_COOLING, []() {
    if (g_subCoolingStartMs[1] == 0)
      return;
    if ((uint32_t)(millis() - g_subCoolingStartMs[1]) < DRY_COOL_MS)
      return;
