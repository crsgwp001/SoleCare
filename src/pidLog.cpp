...
void pidLogInit() {
  ...
  Serial.println("time_ms,ah0,ah1,ah2,s0_diff,s0_wet,s0_state,s0_rate,s0_pid,s1_diff,s1_wet,s1_state,s1_rate,s1_pid");
}
...
void pidLogData(float ah0, float ah1, float ah2,
  ...
  Serial.printf("%lu,%.3f,%.3f,%.3f,%.3f,%s,%s,%.4f,%.3f,%.3f,%s,%s,%.4f,%.3f\n",
                millis(),
                ah0, ah1, ah2,
