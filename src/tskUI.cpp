...
DisplayUnit oled1(23, 21, 0); // SDA, SCL, Wire0
DisplayUnit oled2(22, 19, 1); // SDA, SCL, Wire1
...
void vMainOledTask(void * /*pv*/) {
  ...
  while (true) {
    ...
    const char *sss1 = (ss1 == SubState::S_WET)       ? "WET"
                       : (ss1 == SubState::S_COOLING) ? "COOL"
                       : (ss1 == SubState::S_DRY)     ? "DRY"
                       : (ss1 == SubState::S_DONE)    ? "DONE"
                                                      : "IDLE";
    const char *sss2 = (ss2 == SubState::S_WET)       ? "WET"
                       : (ss2 == SubState::S_COOLING) ? "COOL"
                       : (ss2 == SubState::S_DRY)     ? "DRY"
...
void vAuxOledTask(void * /*pv*/) {
  ...
  while (true) {
    ...
    if (isnan(g_dhtAH_ema[0]))
      snprintf(ah0s, sizeof(ah0s), "--");
    else
      snprintf(ah0s, sizeof(ah0s), "%.2f", g_dhtAH_ema[0]);
    if (isnan(g_dhtAH_ema[1]))
      snprintf(ah1s, sizeof(ah1s), "--");
    else
      snprintf(ah1s, sizeof(ah1s), "%.2f", g_dhtAH_ema[1]);
    if (isnan(g_dhtAH_ema[2]))
      snprintf(ah2s, sizeof(ah2s), "--");
    else
      snprintf(ah2s, sizeof(ah2s), "%.2f", g_dhtAH_ema[2]);
    if (isnan(g_dhtAHDiff[0]))
      snprintf(d0s, sizeof(d0s), "--");
    else
      snprintf(d0s, sizeof(d0s), "%+.2f", g_dhtAHDiff[0]);
    if (isnan(g_dhtAHDiff[1]))
      snprintf(d1s, sizeof(d1s), "--");
    else
      snprintf(d1s, sizeof(d1s), "%+.2f", g_dhtAHDiff[1]);

    char msg[128];
    // Line1: AH for sensors 0/1 and shoe status for shoe0
    // Line2: AH for sensor2, diffs and shoe1 status
    snprintf(msg, sizeof(msg), "AH0:%6s AH1:%6s S0:%-4s\nAH2:%6s Δ1:%6s S1:%-4s", ah0s, ah1s, shoe0,
             ah2s, d0s, shoe1);
