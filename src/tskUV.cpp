// tskUV.cpp - single clean UV task implementation
#include "tskUV.h"
#include "tskFSM.h"
#include <Arduino.h>
...
static constexpr int UV_PIN_0 = 18;
static constexpr int UV_PIN_1 = 5;
static constexpr bool RELAY_ACTIVE_LOW = false; // change to true if relay is active-low
static constexpr uint32_t UV_DEFAULT_MS = 10u * 1000u;
...
static void uvTask(void* pv) {
  ...
  for (;;) {
    ...
    for (int i = 0; i < 2; ++i) {
      bool expired = false;
      taskENTER_CRITICAL(&g_uvMux);
      if (g_uvStarted[i] && g_uvEndMs[i] != 0 && (int32_t)(now - g_uvEndMs[i]) >= 0) {
        g_uvStarted[i] = false;
        g_uvEndMs[i] = 0;
        expired = true;
      }
