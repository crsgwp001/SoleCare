...
// when running and not paused, g_uvEndMs = absolute end time; when paused, g_uvRemainingMs stores remaining time
  ...
  for (;;) {
    if (xQueueReceive(g_uvQ, &msg, pdMS_TO_TICKS(200)) == pdTRUE) {
      ...
      if (msg.cmd == UVCmd::Start) {
        ...
        taskENTER_CRITICAL(&g_uvMux);
        g_uvStarted[i] = false; g_uvPaused[i] = false; g_uvEndMs[i] = 0; g_uvRemainingMs[i] = 0;
        taskEXIT_CRITICAL(&g_uvMux);
        setRelay(i, false);
    ...
    for (int i = 0; i < 2; ++i) {
      bool expired = false;
      taskENTER_CRITICAL(&g_uvMux);
      if (g_uvStarted[i] && !g_uvPaused[i] && g_uvEndMs[i] != 0 && (int32_t)(now - g_uvEndMs[i]) >= 0) {
        g_uvStarted[i] = false; g_uvEndMs[i] = 0; g_uvRemainingMs[i] = 0; expired = true;
      }
      taskEXIT_CRITICAL(&g_uvMux);
...
bool uvResume(uint8_t idx) {
  ...
  taskEXIT_CRITICAL(&g_uvMux);
  if (ok) setRelay(idx, true);
  return ok;
}

bool uvIsPaused(uint8_t idx) { bool p; taskENTER_CRITICAL(&g_uvMux); p = g_uvPaused[idx]; taskEXIT_CRITICAL(&g_uvMux); return p; }

uint32_t uvRemainingMs(uint8_t idx) {
  uint32_t rem = 0;
  taskENTER_CRITICAL(&g_uvMux);
  if (g_uvStarted[idx]) {
    if (g_uvPaused[idx]) rem = g_uvRemainingMs[idx];
    else {
      uint32_t now = millis();
      if (g_uvEndMs[idx] > now) rem = (uint32_t)(g_uvEndMs[idx] - now);
      else rem = 0;
    }
  }
  taskEXIT_CRITICAL(&g_uvMux);
