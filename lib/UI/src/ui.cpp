...
static void lockDisplay() {
    ...
    if (s_displayMux) xSemaphoreTake(s_displayMux, portMAX_DELAY);
}

static void unlockDisplay() {
    if (s_displayMux) xSemaphoreGive(s_displayMux);
}
...
bool DisplayUnit::begin() {
    ...
    bool ok = true;
    if (!display.begin(SSD1306_SWITCHCAPVCC, 0x3C)) { Serial.printf("OLED on bus %d init failed\n", wireID); ok = false; }
...
void DisplayUnit::showMessage(const String& msg) {
    ...
    display.setCursor(0, 0); display.println(msg); display.display();
    unlockDisplay();
}
...
void DisplayUnit::showMessage(const String& msg, bool uv0, bool uv1, bool uv0p, bool uv1p) {
    ...
    display.clearDisplay(); display.setTextSize(1); display.setTextColor(SSD1306_WHITE);
    display.setCursor(0, 0); display.println(msg);
    ...
    if (uv0) display.fillRect(iconX + 2, yTop + 2, iconW - 4, iconH - 4, SSD1306_WHITE);
    ...
    display.setCursor(iconX - 22, yTop + 1); display.print("UV0");
    // (no uv-complete rendering in this version)

    display.drawRect(iconX, yBottom, iconW, iconH, SSD1306_WHITE);
    if (uv1) display.fillRect(iconX + 2, yBottom + 2, iconW - 4, iconH - 4, SSD1306_WHITE);
    if (uv1p) {
        int bx = iconX + 4;
        int by = yBottom + 2;
        int bh = iconH - 4;
    ...
    display.setCursor(iconX - 22, yBottom + 1); display.print("UV1");
    // (no uv-complete rendering in this version)
